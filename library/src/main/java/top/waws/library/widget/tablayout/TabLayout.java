 ...
 * <p>
 * If you're using a {@link androidx.viewpager.widget.ViewPager} together
...
/**
 *  功能描述: 复制自系统的{@link com.google.android.material.tabs.TabLayout}
 *  主要添加了修改下标的宽度 setIndectorWidth(@Px int indectorWidth)
 *  @className: TabLayout
 *  @author: thanatos
 *  @createTime: 2018/1/17
 *  @updateTime: 2018/1/17 下午5:51
 */
@ViewPager.DecorView
public class TabLayout extends HorizontalScrollView {
    ...
    private int mIndicatorModel = LINE;
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // TODO: 2018/5/31 获取自定义属性
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.TabLayout);

        if (array.hasValue(R.styleable.TabLayout_tabIndicatorWidth)){
            mIndicatorWidth = array.getDimensionPixelSize(R.styleable.TabLayout_tabIndicatorWidth,0);
        }
        if (array.hasValue(R.styleable.TabLayout_tabIndicatorModel)){
            mIndicatorModel = array.getInt(R.styleable.TabLayout_tabIndicatorModel,0);
        }
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                //TODO:修改indicator
                final int width = mIndicatorRight - mIndicatorLeft;
                int nW ;
                if (mIndicatorWidth > 0){
                    nW = width -mIndicatorWidth;
                }else {
                    nW = 0;
                }
                int left = mIndicatorLeft+nW/2;
                int right = mIndicatorRight - nW/2;
                if (mIndicatorModel == LINE){
                    canvas.drawRect(left,getHeight() - mSelectedIndicatorHeight,right,getHeight(),
                            mSelectedIndicatorPaint);
                }else if (mIndicatorModel == TRIANGLE){
                    Path path = new Path();
                    path.moveTo(left,getHeight());
                    path.lineTo(left+(right -left)/2,getHeight() - mSelectedIndicatorHeight);
                    path.lineTo(right,getHeight());
                    path.close();
    ...
    public void setIndicatorModel(@IndicatorModel int model){
        this.mIndicatorModel = model;
    }
     ...
     * 主题校验
     */
    private static class ThemeUtils {
